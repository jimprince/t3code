...
Untick one project, then click the `◎` on a _different_ project. Expected: the menu closes, only that project's threads show, the trigger shows its name plus a filled `◉`. Click the `◉` in the trigger. Expected: the previous 2-of-3 selection returns exactly.

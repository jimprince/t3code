...
In `sync-upstream.yml`, tag the rebased `HEAD` directly instead of committing version
stamps first. Remove the `update-release-package-versions.ts` + `git commit` block from
the push step; keep the lockfile refresh needed for a clean rebase result.

Removes 16 commits from the replay stack immediately and prevents ~2/day accruing.
No release-behaviour change — the release build re-derives the version from the tag name
(§2.2).

**Verification:** dispatch `release.yml` against a tag produced by the modified sync;
built artifact version must match the tag, and `nightly-mac.yml` must be byte-identical
in shape to a pre-change release.

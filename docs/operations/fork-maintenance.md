...
2. **Never commit generated release artifacts to `main`.** Both Sync Upstream and Fork
   Push Nightly use `scripts/ci/prepare-release-tag`: the release tag names the stamped
   child, while `main` is pushed at the exact pre-stamp stack tip. `release.yml` consumes
   that tag and never pushes `main`.
...
7. **Every automated `main` rewrite binds its lease to the checked-out starting HEAD.**
   Before replay/preparation, require live `origin/main` to equal that exact HEAD and
   fail a stale queued checkout rather than renewing it from the remote. Use
   `--force-with-lease=refs/heads/main:<starting-head>`. A later fetch must not renew the
   lease. Push the release tag only after the leased `main` push succeeds.

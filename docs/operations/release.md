...
(`scripts/ci/fork-topics.json`) onto the selected upstream tag with
`git rebase --autosquash`, stamps package versions to the derived fork release
version via `scripts/ci/prepare-release-tag`, and pushes the release tag at
that stamped commit while `main` stays at the pre-stamp SHA. `--autosquash`
folds integration `fixup!` commits into the topic they repair, and
`commit-discipline` (`.github/workflows/ci.yml`) enforces that no new
top-level commit accumulates on `main` — this is what keeps the next rebase
bounded, not collapsing history into one commit.

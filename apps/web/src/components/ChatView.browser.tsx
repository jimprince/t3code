...
function createMockEnvironmentApi(input: {
  ...
  return {
    ...
    orchestration: {
      ...
      exportThread: (() => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["exportThread"],
      importThread: (() => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["importThread"],
    },
    preview: {
      open: () => {
        throw new Error("Not implemented in browser test.");
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("falls back to current checkout when a worktree draft has no valid base ref", async () => {
    useComposerDraftStore.setState({
      draftThreadsByThreadKey: {
        [THREAD_KEY]: {
          threadId: THREAD_ID,

  ...
  const getThreadDetailByIdBounded = (threadId: ThreadId, bounds: ThreadDetailBounds | undefined) =>
    Effect.gen(function* () {
      const [
        ...
        // Upstream's pagination is opt-in, so the unwindowed read is still the
        // unbounded one it always was. The fork caps it: a long-lived thread's
        // activity list is what stalls snapshot handoff on reconnect.
        (bounds === undefined

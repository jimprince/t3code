...
it.effect("getThreadDetailSnapshot returns an atomic thread detail cut point", () =>
  Effect.promise(async () => {
    ...
    try {
      ...
      const snapshot = await harness.run(harness.snapshotQuery.getThreadDetailSnapshot(threadId));
        ...
        const deliveredEvent = await harness.run(
          Effect.scoped(
            Effect.gen(function* () {
              ...
              const snapshot = yield* harness.snapshotQuery.getThreadDetailSnapshot(threadId);
              assert.equal(snapshot._tag, "Some");

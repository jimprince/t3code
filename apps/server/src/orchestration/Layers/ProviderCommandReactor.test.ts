...
describe("ProviderCommandReactor", () => {
      ...
      input?.importedHistory === true
        ? engine.dispatch({
            ...
            thread: {
              ...
              title: input?.initialTitle ?? "Imported thread",
              modelSelection,
  ...
  effectIt.effect(
    "sends legacy file metadata through native provider attachments and imports old tmp bytes",
    () =>
      Effect.gen(function* () {
        const started = yield* Deferred.make<void>();
        const harness = yield* Effect.promise(() =>
        ...
        expect(message?.fileAttachments).toEqual([
          expect.objectContaining({ id, path: oldTmpPath }),
        ]);
  ...
  effectIt.effect(
    "reuses a durable legacy file after restart when its historical tmp path is gone",
    () =>
      Effect.gen(function* () {
        const started = yield* Deferred.make<void>();
        const harness = yield* Effect.promise(() =>

...
not disable the nightly lane to silence a failed nightly run; fix the replay,
...
- sync-upstream fetches upstream tags into `refs/tags/upstream/*` (namespaced)
  ...
  generated release metadata never lands on `main` and the topic stack never
...
- Release publication uses the already prepared tag/source commit and never
  commits or pushes `main`; there is no post-publication version finalizer.
  Nightly preflight and Linux build installs use `--ignore-scripts` so native
...
## Fork-only push releases

`.github/workflows/fork-push-nightly.yml` watches a narrow allowlist of paths
that can affect packaged app/runtime output (`apps/**`, `packages/**`,
`assets/**`, root package/build files, and desktop artifact build inputs).
Docs, workflows, and unrelated helper scripts do not publish a fork-only
desktop update automatically. The workflow replays the fork onto the newest
upstream nightly, allocates the next `${upstream_nightly_tag}-fork.N`, and
uses `scripts/ci/prepare-release-tag`. The prepared child carries the stamped
versions and release tag; `main` remains the unstamped parent.

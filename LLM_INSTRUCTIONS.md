...
`.github/workflows/ci.yml`, and `scripts/ci/prepare-release-tag` is called from
`sync-upstream.yml` to keep generated release metadata off `main` (see below).
...
- sync-upstream fetches upstream tags into `refs/tags/upstream/*` (namespaced)
  ...
  (`scripts/ci/check-commit-discipline`). `scripts/ci/prepare-release-tag`
  then stamps releasable package versions into a separate prep commit, tags
  it, and reports the pre-stamp SHA; `main` is force-pushed at that pre-stamp
  SHA (not the prep commit), so generated release metadata never lands on
  `main` and the topic stack never grows a per-sync commit.

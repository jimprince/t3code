...
upstream release tags plus the net fork patch replayed on top. There is no
independent versioning axis for the fork.
...
- Normal pushes to `main` that affect packaged app/runtime output run
  ...
  changes through the nightly feed. The workflow replays the fork patch onto the
  ...
  `vNEXT-fork.N` releases. The next upstream stable sync replays the fork patch
  onto the upstream stable tag and publishes the integrated stable
  release.
- sync-upstream fetches upstream tags into `refs/tags/upstream/*` (namespaced)
  to avoid clobbering our tags, then collapses the net fork delta into one
  replay commit on the upstream tag, stamps releasable package versions to the
  fork release version, and
  force-pushes `main` plus a release tag that points at the stamped commit.
  Collapsing on every sync prevents historical conflict-repair commits from
  conflicting again on every nightly and keeps the next replay bounded.
